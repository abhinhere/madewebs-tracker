...
export function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-50 w-64 border-r border-border bg-sidebar p-4 transition-transform duration-300 ease-in-out lg:hidden",
      ...
      >
        ...
        <div className="absolute bottom-4 left-4 right-4 rounded-lg border border-border bg-background p-3 flex flex-col gap-2">
          <div className="min-w-0">
            <p className="text-xs font-semibold text-muted-foreground truncate">{isAdmin ? "Admin access" : "Employee"}</p>
        ...
        <header className="sticky top-0 z-20 border-b border-border bg-background/85 backdrop-blur">
          <div className="flex min-h-16 flex-wrap items-center gap-3 px-4 py-3 lg:px-7">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="truncate text-sm text-muted-foreground hidden sm:block">Projects, payments, reviews, and team delivery in one workspace.</p>
            </div>
            {isAdmin && (
